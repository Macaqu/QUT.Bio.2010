...
namespace QUT.Bio.BioPatML.Statistic
{
    ...
    public class HistogramSymbol
    {
        ...
        public HistogramSymbol(ISequence sequence)
        {
            Add(sequence);
        }
#endregion
        ...
        public void Add(HistogramSymbol histogram, IAlphabet alphabet)
        {
            for (int i = 0; i < histogram.Count; i++)
            {
                char symbol = histogram[i];
                Add(symbol, histogram.HistoValue(i), alphabet);
            }
        }
        ...
        public void Add(char symbol, IAlphabet alphabet)
        {
            Add(symbol, 1, alphabet);
        }
        ...
        public void Add(char symbol, int counts, IAlphabet alphabet)
        {
            Counter counter = null; //did the C# style

            if (DictionaryCounter.ContainsKey(alphabet.Name))
                counter = DictionaryCounter[alphabet.Name];

            if (counter == null)
            {
                counter = new Counter(counts, symbol, list.Count);
                DictionaryCounter.Add(alphabet.Name, counter);
                list.Add(counter);
                sum++;
            }
            else
        ...
        public void Substract(HistogramSymbol histogram, IAlphabet alphabet)
        {
            for (int i = 0; i < histogram.Count; i++)
            {
                char symbol = histogram[i];
        ...
        public int HistoValue(char symbol, IAlphabet alphabet)
        {
        ...
        public double Frequency(char symbol)
        {
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < Count; i++)
                sb.Append(this[i]).Append(":").Append(HistoValue(i)).Append(" ");
            return sb.ToString();
        }
        ...
        private class Counter
        {
            ...
            public Counter
                (int counts, char symbol, int binIndex)
            {
                this.counts = counts;
                this.symbol = symbol;
