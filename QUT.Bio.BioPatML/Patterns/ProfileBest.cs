...
namespace QUT.Bio.BioPatML.Patterns {
	...
	public sealed class ProfileBest : Profile {
		...
		/// <summary> storage for max. match information 
		/// </summary>
		...
		/// <summary> Constructs an empty profile.
		/// </summary>
		...
		private Match MaxMatch ( 
			...
			maxMatch.Set( null, 0, 0, Strand.Unknown, -Double.MaxValue );

			for ( int pos = start; pos <= end; pos++ ) {
				do {
					Match match = pattern.Match( seq, pos );
		...
		public override Match Match ( 
			...
			Match maxMatch = null;
			int patternNumber = profileElements.Count;
			...
			for ( int index = 0; index < patternNumber; index++ ) {
				...
				if ( maxMatch == null )
					return ( null );
				element.Pattern.LatestMatch.Set( maxMatch );
			}
